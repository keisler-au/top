...
**Status: active delivery plan.** This plan first makes the run-scoped batch
...
This is the only current work-package document. Work proceeds strictly in the
listed order. The repository contains partial candidate-taxonomy scaffolding,
but no package below is complete until its acceptance criteria and evidence
requirements have been verified and recorded here.
...
direct successor. Do not archive this document or create another current plan
until every row is complete; follow the lifecycle in [the documentation
guide](README.md#documentation-maintenance-process).
...
- [x] Work — the completed batch programme has concise current-state and
  archive records. The current work-package document remains unarchived as
  required because it is now the sole plan for the still-active PW programme;
  the document lifecycle rule archives it only once those remaining rows are
  complete.
